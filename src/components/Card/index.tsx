...
const Card = ({data}) => {
    const bgCard = data.type === 1 ? 'bg-[#E0E0E0]' : data.type === 2 ?  'bg-[#FAFAFA]' : 'bg-[#212121]'
    const textCardColor = data.type === 3 ? 'text-white' : 'text-[#212121]'
    const bgBtn = data.type === 1 ? 'bg-[#757575]' : data.type === 2 ?  'bg-[#5043FF]' : 'bg-[#FAFAFA]'
    const textBtnColor = data.type === 3 ? 'text-[#212121]' : 'text-white' 

    return (
        <div className={`flex flex-col h-[560px] items-start  w-[344px] ${bgCard} rounded-[64px] p-8 ${data.type===2 && 'border-2 border-[#5043FF] shadow-[0_10px_24px_rgba(80,67,255,0.24)]'}`}>
            <div className="flex h-[204px] flex-col gap-3">
                ...
                {data.type!==3 && <div className="flex items-end justify-start">
            ...
            <div className="w-full">
                    {data.includes.map((item ,index)=>(
                         <div className="flex justify-between items-center mb-3" key={index}>
                            ...
                            <Image src="/questionCircle.svg" alt="questionCircle" width={16} height={16}/>
                        </div>
                    ))}
            </div>
            {data.type === 2 && <div className="flex items-center justify-center w-full mt-5">
